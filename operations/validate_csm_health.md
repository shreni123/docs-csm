...
- [4. NCN gateway and SSH access health checks](#4-ncn-gateway-and-ssh-access-health-checks)
...
## 4. NCN gateway and SSH access health checks
...
The SSH access tests check the accessibility of SSH to and from various node types over all the relevant networks. There are two
types of SSH access tests: internal and external. Internal tests will start from an NCN, will login to various node types, and from
those node types check whether other node types are accessible via SSH on networks where they should be accessible, and NOT accessible
on networks where they should not be accessible. Similarly, external tests will start from outside the cluster, and check whether
various node types are SSH-accessible over various networks from outside the cluster correspondingly.
